...
for (const depName of Object.keys(dependencies)) {
  ...
  if (importantDeps.includes(lower)) {
    tech.add(depName);
  }
}
    } catch {
...
export async function analyzeRepository({
  ...
  isInitialAnalysis ? "__INIT__" : question;
  ...
  const repositoryStructure = buildProjectStructure(
  snapshot.files,
  selectedPaths
);
  ...
  // detect tech stack from tree + package.json
  ...
  isInitialAnalysis ? "Repository overview" : question;
  ...
  try {
    ...
    const effectiveQuestionForLLM =
      isInitialAnalysis ? "Analyze this repository" : question;
    ...
    for (const field of requiredTextFields) {
      if (typeof llmResult[field] !== "string" || llmResult[field].trim().length === 0) {
        llmResult[field] = fallback[field];
      }
    }

    const safeNotes = ensureStringArray(llmResult.notes);

    if (!isValidQuality(llmResult.quality)) {
      llmResult.quality = fallback.quality;
      llmResult.notes = [
        ...safeNotes,
        "Quality scores derived from fallback (LLM incomplete)",
      ];
    } else {
      llmResult.notes = safeNotes;
    }

    const llmSampleQuestions = ensureStringArray(llmResult.sampleQuestions);
    if (llmSampleQuestions.length === 0) {
      llmResult.sampleQuestions = ensureArray(fallback.sampleQuestions);
    } else {
      llmResult.sampleQuestions = llmSampleQuestions;
    }

    llmResult.techStack = ensureArray(llmResult.techStack, fallback.techStack);
